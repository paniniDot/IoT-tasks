...
void setup()
{
  ...
  t0->init(2 * SCHED_PERIOD);
  t1->init(2 * SCHED_PERIOD);
  t2->init(2 * SCHED_PERIOD);
  t3->init(2 * SCHED_PERIOD);
  t4->init(2 * SCHED_PERIOD);
  t5->init(20 * SCHED_PERIOD);
  t6->init(2 * SCHED_PERIOD);
  t7->init(2 * SCHED_PERIOD);
  sched.addTask(t2);
  sched.addTask(t0);
  sched.addTask(t6);
  sched.addTask(t7);
